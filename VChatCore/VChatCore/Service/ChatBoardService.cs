...
namespace VChatCore.Service
{
    public class ChatBoardService
    {
        ...
        public void DeleteAllMessage(string groupCode)
        {
            Group group = this.context.Groups.FirstOrDefault(x => x.Code.Equals(groupCode));

            if (group != null)
            {
                this.context.Messages.RemoveRange(group.Messages);
                this.context.SaveChanges();
                try
                {
                    this.chatHub.Clients.All.SendAsync("messageHubListener", true);
        ...
        /// </summary>
        /// <param name="groupCode"></param>
        public void LeaveGroup(string userCode, string groupCode)
        {
            GroupUser groupUser = this.context.GroupUsers.FirstOrDefault(x => x.UserCode.Equals(userCode) && x.GroupCode.Equals(groupCode));
            ...
            if (groupUser != null)
            {
                ...
                User sendTo = this.context.Users.FirstOrDefault(x => x.Code.Equals(userCode));

                Message msg = new Message()
                {
                    Content = sendTo.FullName + " đã rời khỏi nhóm trò chuyện",
                    Created = dateNow,
                    CreatedBy = userCode,
                    GroupCode = grp.Code,
                    Type = "text",

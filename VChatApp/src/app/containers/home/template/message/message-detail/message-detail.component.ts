...
export class MessageDetailComponent implements OnInit {
  ...
  addEmoji(icon: string) {
    this.textMessage = this.textMessage + " " + icon;
    this.sendMessage();
  }
  ...
  deleteAllMessage() {
    if (confirm("Xác nhận xóa")) {
      this.chatBoardService.deleteAllMessage(this.group?.Code ?? "")
        .subscribe((resp: any) => {
        }, (error) => {
          console.log(error)
        });
  ...
  leaveGroup() {
    if (confirm("Xác nhận rời khỏi nhóm")) {
      this.chatBoardService.leaveGroup(this.group?.Code ?? "")
        .subscribe((resp: any) => {
          location.reload()
        }, (error) => {
          console.log(error)

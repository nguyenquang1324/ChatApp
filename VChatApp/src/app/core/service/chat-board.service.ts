...
export class ChatBoardService {
  ...
  deleteAllMessage(groupCode: string) {
    return this.http.delete(AppRoutingApi.DeleteAllMessage + "/" + groupCode);
  }

  leaveGroup(groupCode: string) {
    return this.http.delete(AppRoutingApi.LeaveGroup + "/" + groupCode);
  }
}
